...
void TestMemory::memory_read_ctl() {
    ...
    QCOMPARE(bus.read_ctl(AC_U64, frontend_address + stride).i.as_u64(), result.u64);

    for (size_t i = 0; i < 2; ++i) {
        QCOMPARE(
            bus.read_ctl(AC_U32, frontend_address + stride + 4 * i).i.as_u32(), result.u32.at(i));
    }
    for (size_t i = 0; i < 4; ++i) {
        QCOMPARE(
            bus.read_ctl(AC_U16, frontend_address + stride + 2 * i).i.as_u16(), result.u16.at(i));
    }
    for (size_t i = 0; i < 4; ++i) {
        QCOMPARE(
            bus.read_ctl(AC_I16, frontend_address + stride + 2 * i).i.as_i16(),
            (int16_t)result.u16.at(i));
    }
    for (size_t i = 0; i < 8; ++i) {
        QCOMPARE(bus.read_ctl(AC_U8, frontend_address + stride + i).i.as_u8(), result.u8.at(i));
    }
    for (size_t i = 0; i < 8; ++i) {
        QCOMPARE(
            bus.read_ctl(AC_I8, frontend_address + stride + i).i.as_i8(), (int8_t)result.u8.at(i));
    }
}
