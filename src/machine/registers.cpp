...
uint8_t Registers::read_vl() const {
    // printf("read_vl: %u\n", this->vl);
    return this->vl;
}

void Registers::write_vl(uint8_t value) {
    this->vl = value;
    // printf("write_vl: %u\n", this->vl);
}
...
VectorRegisterValue Registers::read_vr(RegisterId reg) const {
    if (reg == 0) {
        return VectorRegisterValue(); // $0 always reads as 0
    }

    return this->vr.at(reg);
}

void Registers::write_vr(RegisterId reg, VectorRegisterValue value) {
    if (reg == 0) {
        return; // Skip write to $0
    }
...
void Registers::reset() {
    ...
    for (int i = 1; i < 32; i++) {
        write_gp(i, 0);
        write_vr(i, VectorRegisterValue());
    }
